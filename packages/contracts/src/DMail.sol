...
contract DMail {
    ...
    function sendMessage(address recipient, string memory messageHash) public {
        mailboxes[recipient].unread.push(
            Message({
                sender: msg.sender,
    ...
    function getUnreadCount() public view returns (uint256 unreadCount) {
    ...
    function getArchiveAddress()
        public
        view
        returns (string memory archiveAddress)
    {
        return mailboxes[msg.sender].archiveAddress;
    }

    function getMail(uint256 index)
        public
        view
        returns (
            address sender,
